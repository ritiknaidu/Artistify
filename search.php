    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-10 col-md-10 col-sm-10">
                <?php
                    $query = mysqli_query($db,"SELECT * from posts LEFT JOIN users on users.`id` = posts.`user-id` WHERE (`title` LIKE '%".$key."%') OR (`username` LIKE '%".$key."%') OR (`description` LIKE '%".$key."%') OR (`content` LIKE '%".$key."%')") or die(mysqli_error($db));
                    if($result = mysqli_num_rows($query) == 0){ 
                ?>
                        <div class="header">No Result found for '<?php echo $key ?>'</div>
                <?php } 
                    if($result = mysqli_num_rows($query) > 0){ ?>
                        <div class="header">Search Results for '<?php echo $key ?>' are : </div>
                <?php
                    while($row = mysqli_fetch_array($query)) {
                ...
                ?>
                <a href="view_post.php<?php echo '?id='.$post_id; ?>">
